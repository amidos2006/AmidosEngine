...
:desktop-config
set CERT_FILE=%DESKTOP_CERT_FILE%
set SIGNING_OPTIONS=%DESKTOP_SIGNING_OPTIONS%
set ICONS=%DESKTOP_ICONS%
set DIST_EXT=air
set TYPE=air
goto start
...
:: Written Target
set W_TARGET=-target %TYPE%%TARGET% 
if "%TYPE%"=="air" set W_TARGET=
:: Output file
...
echo.
call adt -package %W_TARGET% %OPTIONS% %SIGNING_OPTIONS% "%OUTPUT%" "%APP_XML%" %FILE_OR_DIR%
echo.
if errorlevel 1 goto failed
if NOT "%DEPLOY%" == "" call adt -package -target native "%DIST_PATH%\%DIST_NAME%.%DEPLOY%" "%OUTPUT%"
if errorlevel 1 goto failed
goto end
